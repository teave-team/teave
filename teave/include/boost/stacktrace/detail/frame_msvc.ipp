...
#   pragma comment(lib, "Dbgeng.lib")
    ...
    __CRT_UUID_DECL(IDebugApp,0x27fe5639,0x8407,0x4f47,0x83,0x64,0xee,0x11,0x8f,0xb0,0x8a,0xc8)
    ...
    DEFINE_GUID(IID_IDebugApp,0x27fe5639,0x8407,0x4f47,0x83,0x64,0xee,0x11,0x8f,0xb0,0x8a,0xc8);
...
class debugging_symbols: boost::noncopyable {
    static void try_init_com(com_holder< ::IDebugSymbols>& idebug, const com_global_initer& com) BOOST_NOEXCEPT {
        com_holder< ::IDebugApp> iapp(com);
        if (S_OK != ::DebugCreate(__uuidof(IDebugApp), iapp.to_void_ptr_ptr())) {
            return;
        }

        com_holder< ::IDebugControl> icontrol(com);
        const bool res0 = (S_OK == iapp->QueryInterface(
        ...
        const bool res1 = (S_OK == iapp->AttachProcess(
            0,
            ::GetCurrentProcessId(),
        ...
        iapp->QueryInterface(__uuidof(IDebugSymbols), idebug.to_void_ptr_ptr());
    }
